...
<link rel="stylesheet" type="text/css" href="${APP_PATH }/static/manage/static/h-ui.admin/css/style.css" /><title>资讯列表</title>
</head>
<body>
...
<div class="pd-20">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>
	 <a class="btn btn-primary radius" onclick="article_add('添加资讯','newsadd')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加资讯</a></span> <span class="r">共有数据：<strong>54</strong> 条</span> </div>
	<div class="mt-20">
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<c:forEach items="${pageInfo.list }" var="news">
						<tr class="text-c">
							...
							<td>中北影院</td>
							<td>${news.newsTime }</td>
							<td>${news.newsLookcount }</td>
							<td>${news.newsMovie }</td>
							<td class="td-status"><span class="label label-success radius">${news.newsState=="0"?"已发布":"下架" }</span></td>
							<td class="f-14 td-manage">
							<a style="text-decoration:none" onClick="article_stop(this,'${news.newsId }')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a>
							<a style="text-decoration:none" class="ml-5" onClick="article_edit('资讯编辑','article-add.html','${news.newsId }')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
							<a style="text-decoration:none" class="ml-5" onClick="article_del(this,'${news.newsId }')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
function datadel(){
	...
	$.each($(".check_item:checked"),function(){
		newsTitle += $(this).parents("tr").find("td:eq(2)").text()+",";
		del_idstr += $(this).parents("tr").find("td:eq(1)").text()+"-";
	});
	newsTitle = newsTitle.substring(0, newsTitle.length-1);
	del_idstr = del_idstr.substring(0, del_idstr.length-1);
	layer.confirm('确认要删除【'+newsTitle+'】吗？',function(index){
		$.ajax({
			type: 'DELETE',
