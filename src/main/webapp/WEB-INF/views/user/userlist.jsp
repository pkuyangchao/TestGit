...
<div class="page-container">
	...
	<div class="text-c"> 日期范围：
		...
		<input type="text" class="input-text" style="width:250px" placeholder="输入用户名称、电话、邮箱" id="" name="keyword">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<c:forEach items="${pageInfo.list }" var="user">
						<tr class="text-c">
							...
							<td>${user.userId }</td>
							<td><u style="cursor:pointer" class="text-primary" onclick="member_show('${user.userAccount }','usershow?userId=${user.userId }','${user.userId }','360','400')">${user.userAccount }</u></td>
							<td>${user.userSex=="0"?"男":"女"  }</td>
							<td>${user.userName }</td>
							<td>${user.userPhone }</td>
							<td>${user.userEmail }</td>
							<td>${user.userType=="0"?"普通会员":"超级会员"  }</td>
							<td>${user.userTime }</td>
							...
							<td class="td-manage">
								...
								<a style="text-decoration:none" class="ml-5" onClick="change_password('修改密码','userchangepassword?userId=${user.userId }','${user.userId }','600','270')" href="javascript:;" title="修改密码"><i class="Hui-iconfont">&#xe63f;</i></a>
