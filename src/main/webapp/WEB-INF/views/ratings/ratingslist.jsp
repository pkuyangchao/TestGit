...
<div class="page-container">
	<div class="text-c"> 日期范围：
		...
		<input type="text" class="input-text" style="width:250px" placeholder="输入用户编号" id="" name="">
		<button type="submit" class="btn btn-success radius" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜用户</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> </span> <span class="r">共有数据：<strong>${pageInfo.total }</strong> 条</span> </div>
	<div class="mt-20">
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
		<c:forEach items="${pageInfo.list }" var="ratings">
						<tr class="text-c">
							<td><input class="check_item" type="checkbox" value="1" name=""></td>
							<td>${ratings.ratingsId }</td>
							<td><u style="cursor:pointer" class="text-primary" onclick="member_show('${ratings.userId }','usershow?userId=${ratings.userId }','${ratings.userId }','360','400')">${ratings.userId }</u></td>
							<td><u style="cursor:pointer" class="text-primary" onclick="member_show('${ratings.movieId }','movieshow?movieId=${ratings.movieId }','${ratings.movieId }','560','600')">${ratings.movieId }</u></td>
							<td>${ratings.preference}</td>
							<td>${ratings.ratingsTime }</td>
							<td class="td-status"><span class="label label-success radius">已启用</span></td>
							<td class="td-manage">
								<a style="text-decoration:none" onClick="member_stop(this,'10001')" href="javascript:;" title="停用"><i class="Hui-iconfont">&#xe631;</i></a>
								<a title="删除" href="javascript:;" onclick="member_del(this,'${ratings.ratingsId }')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
							</td>
						</tr>
			</c:forEach>		
		</tbody>
...
function member_del(obj,id){
	var userAccount = $(obj).parents("tr").find("td:eq(1)").text();
	layer.confirm('确认要删除【'+userAccount+'】吗？',function(index){
		$.ajax({
			type: 'DELETE',
			url: '${APP_PATH}/ratingsId/'+id,
			dataType: 'json',
			success: function(data){
...
function datadel(){
	...
	layer.confirm('确认要删除【'+userAccounts+'】吗？',function(index){
		$.ajax({
			type: 'DELETE',
			url: '${APP_PATH}/ratings/'+del_idstr,
			dataType: 'json',
			success: function(data){
				layer.msg('已删除!',{icon:1,time:1000});
				location.replace(location.href);
